...
if (window.__WSS_INJECTED__) {
  // Already injected; avoid attaching duplicate listeners
  // You can still accept updateConfig messages because they use chrome.runtime.onMessage which is global.
} else {
  window.__WSS_INJECTED__ = true;
  ...
  const SHIFTENTER_SEND_DOMAINS = ['chat.deepseek.com', 'deepseek.com', 'lmarena.ai', 'www.lmarena.ai'];
  ...
  // --- Storage Load (hot-start defaults) ---
  chrome.storage.local.get(
    ['enabled','wordChars','highlightColor','separator','altDragEnabled','multiSelectEnabled','enterOverrideEnabled'],
    (res) => {
      Object.assign(config, {
        ...
        enterOverrideEnabled: (res.enterOverrideEnabled !== undefined) ? !!res.enterOverrideEnabled : config.enterOverrideEnabled
      });
      compileWordRegex();
  ...
  // --- Messaging: updateConfig + getSelections ---
  chrome.runtime.onMessage.addListener((message) => {
    ...
    if (message.type === 'updateConfig') {
      ...
      if ('enterOverrideEnabled' in message) config.enterOverrideEnabled = !!message.enterOverrideEnabled;

      compileWordRegex();
      ensureHighlightStyle();
      repaintHighlights();

      if ('enabled' in message && message.enabled === false) {
        cleanupAllUI();
      }
      refreshEnterHooks();
      return;
  ...
  // --- Modifier keys + Lock + Escape clear ---
  ...
  // ---------- Chat Enter Override (toggle-aware) ----------

  let shiftEnterDocHandlerAttached = false;
  let shiftEnterDocHandler = null;
  let chatgptObserver = null;

  function attachShiftEnterDocHandler() {
    if (shiftEnterDocHandlerAttached) return;
    shiftEnterDocHandler = function(e) {
      if (!config.enabled || !config.enterOverrideEnabled) return;
      if (!domainMatches(SHIFTENTER_SEND_DOMAINS)) return;
    ...
    document.addEventListener('keydown', shiftEnterDocHandler, true);
    shiftEnterDocHandlerAttached = true;
  }
  function detachShiftEnterDocHandler() {
    if (!shiftEnterDocHandlerAttached) return;
    document.removeEventListener('keydown', shiftEnterDocHandler, true);
    shiftEnterDocHandler = null;
    shiftEnterDocHandlerAttached = false;
  }

  function setupChatGPTEnterOverride() {
    if (!isChatGPTDomain()) return;
    document.querySelectorAll('div.ProseMirror[contenteditable="true"]').forEach(attachChatGPTHandler);

    if (!chatgptObserver) {
      chatgptObserver = new MutationObserver((muts) => {
        for (const m of muts) {
          m.addedNodes && m.addedNodes.forEach(n => {
            if (n.nodeType === 1) {
              if (n.matches?.('div.ProseMirror[contenteditable="true"]')) attachChatGPTHandler(n);
              n.querySelectorAll?.('div.ProseMirror[contenteditable="true"]').forEach(attachChatGPTHandler);
            }
          });
        }
      });
  ...
  function attachChatGPTHandler(ed) {
    ...
    ed.__wss_enter_handler = handler;
    ed.addEventListener('keydown', handler, true);
    ed.addEventListener('keydown', handler, false);
  }

  function detachChatGPTEnterOverride() {
    document.querySelectorAll('div.ProseMirror[contenteditable="true"]').forEach(ed => {
      const h = ed.__wss_enter_handler;
      if (h) {
        ed.removeEventListener('keydown', h, true);
        ed.removeEventListener('keydown', h, false);
        ed.__wss_enter_handler = null;
        ed.__WSS_ENTER_PATCHED = false;
      }
    });
    if (chatgptObserver) {
  ...
  function refreshEnterHooks() {
    if (!config.enabled || !config.enterOverrideEnabled) {
      detachShiftEnterDocHandler();
      detachChatGPTEnterOverride();
      return;
    }
    attachShiftEnterDocHandler();
    setupChatGPTEnterOverride();
  }

  // Insert a visible line break into ProseMirror editor
  ...
  function dispatchEditorInsert(el, type) {
    try {
      ...
      if (!notCanceled || before.defaultPrevented) return true;
  ...
  function placeCaretAfterLastNode(root) {
    try {
      const sel = window.getSelection();
      if (!sel) return;
      const r = document.createRange();
      ...
      if (node && node.nodeType === Node.TEXT_NODE) r.setStart(node, node.textContent.length);
      else if (node) r.setStartAfter(node);
  ...
  // --- Cleanup/refresh helpers ---
  function cleanupAllUI() {
    try { if (CSS.highlights) CSS.highlights.delete(HIGHLIGHT_NAME); } catch {}
    removeOverlayHighlights();
    clearMultiSelection();
  }
